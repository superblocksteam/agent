...
- Add runtime configuration and token-stable connection pooling support for Postgres IAM role authentication while preserving legacy password pooling. Operators set `SUPERBLOCKS_POSTGRES_IAM_ALLOWED_ROLE_ARN_PREFIXES` to a JSON array of exact role ARNs or trailing-slash role-path prefixes; Helm accepts the equivalent YAML list, and the empty default denies all roles.
- Pin On-Premise Agent Helm Native Database module defaults to `terraform-superblocks-databases` `v0.3.5` (IAM ACL bootstrap SET ROLE fix; override module `source` URLs to `?ref=latest` for Superblocks EE/internal testing).
...
## v1.42.0
- Reject Apps 3.0 (`FetchCode`) execution requests when the requested profile key is not covered by this agent's `agent.tags` profile data tags (or legacy `agent.environment` fallback). Apps 3.0 requests can now carry both the human-facing profile display name and canonical profile key; validation uses the key when present and falls back to the prior name/environment fields for compatibility.
