...
// Direct port of https://github.com/cloudevents/sdk-go/blob/main/binding/format/protobuf/v2/protobuf.go
// Need this since types are different
import (
	"context"
	"errors"
	"fmt"
...
const (
	...
	ContentTypeProtoJson = "application/proto-json"
...
func init() {
	datacodec.AddEncoder(ContentTypeProtoJson, func(ctx context.Context, in any) ([]byte, error) {
		proto, ok := in.(protoreflect.ProtoMessage)
		if !ok {
			return nil, errors.New("could not encode: " + ContentTypeProtoJson + " required")
		}
		return protojson.Marshal(proto)
	})
	datacodec.AddDecoder(ContentTypeProtoJson, func(ctx context.Context, data []byte, v any) error {
		protoMessage, ok := v.(protoreflect.ProtoMessage)
		if !ok {
			return errors.New("could not decode: " + ContentTypeProtoJson + " required")
		}
		return protojson.Unmarshal(data, protoMessage)
	})
}

func FromProto(container *eventv2.CloudEvent) (*cloudevents.Event, error) {
	...
	if container.Attributes != nil {
		...
		if attr != nil {
			if stattr, ok := attr.Attr.(*eventv2.CloudEventAttributeValue_CeString); ok {
				contentType = stattr.CeString
			}
		}
	}
	switch dt := container.Data.(type) {
	case *eventv2.CloudEvent_BinaryData:
		...
		// }
	case *eventv2.CloudEvent_TextData:
	...
	case *eventv2.CloudEvent_ProtoData:
...
func ToProto(e *event.Event) (*eventv2.CloudEvent, error) {
	container := &eventv2.CloudEvent{
		Id:          e.ID(),
		Source:      e.Source(),
		SpecVersion: e.SpecVersion(),
		Type:        e.Type(),
		Attributes:  make(map[string]*eventv2.CloudEventAttributeValue),
	}
	...
	container.Data = &eventv2.CloudEvent_BinaryData{
		BinaryData: e.Data(),
	}
	if e.DataContentType() == ContentTypeProtobuf {
		...
		container.Data = &eventv2.CloudEvent_ProtoData{
			ProtoData: anymsg,
		}
	}
...
func Unmarshal(b []byte, e *event.Event) error {
	pbe := &eventv2.CloudEvent{}
...
func valueFrom(attr *eventv2.CloudEventAttributeValue) (interface{}, error) {
	var v interface{}
	switch vt := attr.Attr.(type) {
	case *eventv2.CloudEventAttributeValue_CeInteger:
		v = vt.CeInteger
	case *eventv2.CloudEventAttributeValue_CeString:
		v = vt.CeString
	case *eventv2.CloudEventAttributeValue_CeBytes:
		v = vt.CeBytes
	case *eventv2.CloudEventAttributeValue_CeUri:
		uri, err := url.Parse(vt.CeUri)
		if err != nil {
			return nil, fmt.Errorf("failed to parse URI value %s: %s", vt.CeUri, err.Error())
		}
		v = uri
	case *eventv2.CloudEventAttributeValue_CeUriRef:
	...
	case *eventv2.CloudEventAttributeValue_CeTimestamp:
...
func attributeFor(v interface{}) (*eventv2.CloudEventAttributeValue, error) {
	vv, err := types.Validate(v)
	if err != nil {
		return nil, err
	}
	attr := &eventv2.CloudEventAttributeValue{}
	switch vt := vv.(type) {
	case bool:
		attr.Attr = &eventv2.CloudEventAttributeValue_CeBoolean{
			CeBoolean: vt,
		}
	case int32:
		attr.Attr = &eventv2.CloudEventAttributeValue_CeInteger{
			CeInteger: vt,
		}
	case string:
		attr.Attr = &eventv2.CloudEventAttributeValue_CeString{
			CeString: vt,
		}
	case []byte:
		attr.Attr = &eventv2.CloudEventAttributeValue_CeBytes{
			CeBytes: vt,
		}
	case types.URI:
		attr.Attr = &eventv2.CloudEventAttributeValue_CeUri{
			CeUri: vt.String(),
		}
	case types.URIRef:
		attr.Attr = &eventv2.CloudEventAttributeValue_CeUriRef{
			CeUriRef: vt.String(),
		}
	case types.Timestamp:
		attr.Attr = &eventv2.CloudEventAttributeValue_CeTimestamp{
