import { sendUnaryData, ServerDuplexStream, ServerUnaryCall, ServerWritableStream, UntypedHandleCall } from '@grpc/grpc-js';
...
export class EmbeddedExecutorServiceServer implements IExecutorServiceServer {
      ...
      if (receive.hasExecute()) {
        ...
        for (let i = 0; i < mockList.length; i++) {
          ...
          if (on.hasDynamic()) {
            ...
            const shouldMock = await this.requestFuncEvaluation(
              funcName,
              [{ stepName: 'step1', configuration: { host: 'localhost', port: '5432' } }],
              stream
            );
  ...
  // @ts-ignore
  async requestFuncEvaluation(
    funcName: string,
    parameters: any[],
    stream: ServerDuplexStream<TwoWayRequest, TwoWayResponse>
  ): Promise<any> {
