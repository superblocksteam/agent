...
describe('e2e', () => {
  ...
  it('e2e', async () => {
    ...
    const mocks: Mock[] = [postgresMock1, mysqlMock1];
    ...
    expect(res.getOutput().getResult().toJavaScript()).toEqual({ date: '2027-01-01', 'faraday-future': '1000', tesla: '0' });

    expect(
      res
        .getEvents()
        .filter((e) => e.hasEnd())
        .map((e) => e.getEnd().getOutput().getResult().toJavaScript())
    ).toEqual([
      { date: '2026-01-01', 'faraday-future': '100', tesla: '10' },
    ...
    expect(executionServiceServer.getDynamicMockMatchers()).toEqual([true, undefined]);

    expect(executionServiceServer.getStaticMockReturns()).toEqual([
      [{ date: '2026-01-01', 'faraday-future': '100', tesla: '10' }],
      undefined
    ]);
    expect(executionServiceServer.getDynamicMockReturns()).toEqual([undefined, { hostName: 'localhost', username: 'postgres' }]);
