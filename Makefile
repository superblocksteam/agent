...
deploy-helm:
	...
	helm upgrade -i --wait --timeout $(HELM_TIMEOUT) -n $(K8S_NAMESPACE) orchestrator helm/orchestrator \
		...
		--set ephemeral_worker.superblocks.key="$(HELM_SUPERBLOCKS_KEY)" \
		--set ephemeral_worker.queue.host="$(HELM_QUEUE_HOST)" \
		--set ephemeral_worker.queue.servername="$(HELM_QUEUE_HOST)" \
		--set ephemeral_worker.queue.password="$(HELM_QUEUE_TOKEN)" \
		--set ephemeral_worker.kvstore.host="$(HELM_KVSTORE_HOST)" \
		--set ephemeral_worker.kvstore.servername="$(HELM_KVSTORE_HOST)" \
	...
	@if [ "$(ENVIRONMENT)" = "prod" ] || [ "$(ENVIRONMENT)" = "prod-eu" ]; then \
		...
		--values helm/orchestrator/$(ENVIRONMENT).yaml \
		--set worker_go.image.tag=$(IMAGE_TAG) \
		--set worker_go.queue.host=$(HELM_QUEUE_VALKEY_HOST) \
		--set worker_go.queue.servername=$(HELM_QUEUE_VALKEY_HOST) \
		--set worker_go.kvstore.host=$(HELM_KVSTORE_VALKEY_HOST) \
		--set worker_go.kvstore.servername=$(HELM_KVSTORE_VALKEY_HOST) \
		--set worker_go.superblocks.key="$(HELM_SUPERBLOCKS_KEY)" \
		--set worker_py.image.tag=$(IMAGE_TAG) \
		...
		--set worker_js.superblocks.privateKeyEd25519="$(HELM_WORKER_KEY_ED25519)" \
		--set ephemeral_worker.superblocks.key="$(HELM_SUPERBLOCKS_KEY)" \
		--set ephemeral_worker.queue.host="$(HELM_QUEUE_VALKEY_HOST)" \
		--set ephemeral_worker.queue.servername="$(HELM_QUEUE_VALKEY_HOST)" \
		--set ephemeral_worker.queue.password="$(HELM_QUEUE_TOKEN)" \
