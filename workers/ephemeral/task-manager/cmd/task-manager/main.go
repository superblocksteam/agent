...
func init() {
	...
	pflag.StringSlice("worker.plugins.image.use_lang_executor", []string{"javascript", "python"}, "The list of plugins that use the language executor image. Defaults to 'javascript' and 'python'.")
...
func main() {
	...
	pluginsUsingLangExecutor := plugins.Intersection(utils.NewSet(viper.GetStringSlice("worker.plugins.image.use_lang_executor")...))

	var nonLangRequired bool
	{
		if events.Difference(utils.NewSet("execute")).Size() > 0 {
			nonLangRequired = true
		} else if plugins.Difference(pluginsUsingLangExecutor).Size() > 0 {
			nonLangRequired = true
		}
	}
	...
	if events.Contains("execute") && !pluginsUsingLangExecutor.IsEmpty() {
		if pluginsUsingLangExecutor.Size() > 1 {
			fmt.Fprintf(
				os.Stderr,
				"sandbox can only support executions for one language plugin: %s",
				strings.Join(pluginsUsingLangExecutor.ToSlice(), ", "),
			)
			os.Exit(1)
		}

		language = pluginsUsingLangExecutor.ToSlice()[0]
	...
	if nonLangRequired {
		...
		for _, plugin := range plugins.Difference(pluginsUsingLangExecutor).ToSlice() {
