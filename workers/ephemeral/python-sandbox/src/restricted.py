"""Restricted execution environment for sandboxed Python code."""
...
def restricted_import_wrapper(
    import_inclusion_list: list[str] | None = None,
    import_exclusion_list: list[str] | None = None,
...
def build_allowed_builtins(
    import_inclusion_list: list[str] | None = None,
    import_exclusion_list: list[str] | None = None,
    builtins_exclusion_list: list[str] | None = None,
) -> dict[str, Any]:
    """
    ...
    if import_exclusion_list is None:
        import_exclusion_list = SUPERBLOCKS_PYTHON_EXECUTION_IMPORT_DENY_LIST
    if builtins_exclusion_list is None:
...
# Default allowed builtins (no restrictions)
