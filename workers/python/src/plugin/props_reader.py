...
class PluginPropsReader:
    ...
    async def _load_from_store(self):
        ...
        data, size_in_bytes = await self._kv_store.read(keys)
        ...
        if variables is not None:
            ...
            vals = (await self._kv_store.read(keys))[0]
    ...
    def _prepare_read(self):
        execution_id = self._plugin_props.get("executionId")
        binding_keys = self._plugin_props.get("bindingKeys")
        action_configuration = self._plugin_props.get("actionConfiguration")
