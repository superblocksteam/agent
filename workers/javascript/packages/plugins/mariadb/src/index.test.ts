...
const props: PluginExecutionProps<MariaDBDatasourceConfiguration, MariaDBActionConfiguration> = {
  context,
...
describe('MariaDB Plugin', () => {
  ...
  it('test connection', async () => {
    ...
    jest.spyOn(db, 'createConnection').mockImplementation(() => connection);
  ...
  it('execute query', async () => {
    jest.spyOn(client, 'query').mockReturnValue(DUMMY_QUERY_RESULT);
...
AND table_name = ?;`,
        ...
        await queryStartPromise;
        expect(client.query).toHaveBeenLastCalledWith('BEGIN');
        queryStartPromise = makeTestablePromise();
      ...
      it('commits on success', async () => {
        await queryStartPromise;
        expect(client.query).toHaveBeenLastCalledWith(`CREATE TEMPORARY TABLE \`mocktablename\`
(
\`id\` uuid,
\`price\` int4
)`);
        ...
        await queryStartPromise;
        expect(client.query).toHaveBeenLastCalledWith(`INSERT INTO \`mocktablename\` VALUES (?, ?)`, ['a1', 1000]);
        queryStartPromise = makeTestablePromise();
        queryResponsePromise.resolver();
        queryResponsePromise = makeTestablePromise();
        ...
        await queryStartPromise;
        expect(client.query).toHaveBeenLastCalledWith(`ANALYZE TABLE \`mocktablename\``);
        queryStartPromise = makeTestablePromise();
        queryResponsePromise.resolver();
        queryResponsePromise = makeTestablePromise();
...
LEFT JOIN \`public\`.\`products\` ON \`public\`.\`products\`.\`id\` = \`mocktablename\`.\`id\``);
        ...
        await queryStartPromise;
        expect(client.query).toHaveBeenLastCalledWith(`UPDATE \`public\`.\`products\`, \`mocktablename\`
...
LEFT JOIN \`public\`.\`products\` ON \`public\`.\`products\`.\`id\` = \`mocktablename\`.\`id\``);
  ...
  describe('update by any columns', () => {
    describe.each([
      ...
      [{ oldValues: {}, newValues: [] }, "Validation failed, Rows to Filter by is not an array. Given '{}'"],
    ...
    describe('behavior in doUpdate transaction', () => {
      ...
      it('commits on success', async () => {
        ...
        await queryStartPromise;
        expect(client.query).toHaveBeenLastCalledWith(expect.stringContaining('information_schema'), ['public', 'products']);
        queryStartPromise = makeTestablePromise();
        queryResponsePromise.resolver([
          { column_name: 'price', data_type: 'int4' },
...
WHERE \`public\`.\`products\`.\`id\` = \`mocktablename\`.\`id\`;`);
  ...
  describe('insert by primary key', () => {
    describe.each([
      ...
      [
        { table: 'products', insertedRows: null },
        'No rows given. Must provide at least one of Inserted Rows, Updated Rows, or Deleted Rows'
      ],
      [
