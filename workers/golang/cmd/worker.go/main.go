...
func main() {
	...
	var transportRunnable run.Runnable
	{
		...
		transportRunnable = redis.NewRedisTransport(redis.NewOptions(
			...
			redis.WithStreamKeys(redis.StreamKeys(pluginExecutor.ListPlugins(), viper.GetString("worker.group"), []string{viper.GetString("worker.bucket")}, viper.GetStringSlice("worker.events"), false)),
