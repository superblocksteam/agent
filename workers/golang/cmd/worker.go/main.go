...
func init() {
	...
	pflag.String("otel.collector.http.url", "", "The OTLP HTTP collector URL. Empty disables OTLP export.")
...
func main() {
	...
	var transportRunnable run.Runnable
	{
		...
		transportRunnable = redis.NewRedisTransport(redis.NewOptions(
			...
			redis.WithStreamKeys(redis.StreamKeys(pluginExecutor.ListPlugins(), viper.GetString("worker.group"), []string{viper.GetString("worker.bucket")}, nil, utils.GetStringSlice("worker.events"), true)),
